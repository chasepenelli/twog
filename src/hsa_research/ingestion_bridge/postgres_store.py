...
class PostgresResearchRepository(ResearchRepository):
    ...
    def list_entity_aliases(
        ...
        *,
        ...
        if entity_type:
            ...
            params.append(entity_type)
        if query:
            clauses.append("(lower(alias_normalized) like lower(%s) or lower(normalized_key) like lower(%s))")
            params.extend((f"%{query}%", f"%{query}%"))
        ...
        if limit is not None:
            sql += " limit %s"
            params.append(limit)
        return [EntityAlias.model_validate(_payload(row)) for row in self._fetchall(sql, params)]
    ...
    def upsert_source_version(self, record: SourceVersionRecord) -> SourceVersionRecord:
        ...
        row = self._fetchone("select payload from source_versions where source_key = %s", (record.source_key,))
        return SourceVersionRecord.model_validate(_payload(row))
    ...
    def list_source_versions(
        ...
        *,
        ...
        if limit is not None:
            sql += " limit %s"
            params.append(limit)
        return [SourceVersionRecord.model_validate(_payload(row)) for row in self._fetchall(sql, params)]
    ...
    def list_primitive_call_events(
        ...
        *,
        ...
        params: list[object] = []
        if primitive_name:
            clauses.append("primitive_name = %s")
        ...
        if limit is not None:
            sql += " limit %s"
            params.append(limit)
        return [PrimitiveCallEvent.model_validate(_payload(row)) for row in self._fetchall(sql, params)]
    ...
    def get_run_manifest(self, manifest_id: UUID) -> RunManifestRecord | None:
        row = self._fetchone("select payload from run_manifests where manifest_id = %s", (str(manifest_id),))
        if row is None:
            return None
        return RunManifestRecord.model_validate(_payload(row))
    ...
    def list_run_manifests(
        ...
        *,
        ...
        if manifest_type:
            clauses.append("manifest_type = %s")
        ...
        params.append(limit)
        return [RunManifestRecord.model_validate(_payload(row)) for row in self._fetchall(sql, params)]
    ...
    def list_agent_runs(
        ...
        *,
        ...
        if source_key:
            ...
            params.append(source_key)
        if dagster_run_id:
            clauses.append("dagster_run_id = %s")
    ...
    def get_research_workspace(self, workspace_id: UUID) -> ResearchWorkspaceRecord | None:
        row = self._fetchone(
            "select payload from research_workspaces where workspace_id = %s",
            (str(workspace_id),),
        )
        if row is None:
            return None
        return ResearchWorkspaceRecord.model_validate(_payload(row))
    ...
    def list_research_workspaces(
        ...
        *,
        ...
        if statuses:
            ...
            params.append(statuses)
        if skill_profile:
            clauses.append("skill_profile = %s")
    ...
    def get_public_candidate(self, candidate_id: str) -> PublicCandidateRecord | None:
        row = self._fetchone(
            "select payload from public_candidates where candidate_id = %s",
            (candidate_id,),
        )
        if row is None:
            return None
        return PublicCandidateRecord.model_validate(_payload(row))
    ...
    def list_public_candidates(self, request: PublicCandidateLibraryRequest | None = None) -> list[PublicCandidateRecord]:
        ...
        if request.public_status:
            clauses.append("public_status = %s")
        ...
        if clauses:
            sql += " where " + " and ".join(clauses)
        sql += " order by priority_score desc, updated_at desc limit %s"
        params.append(request.limit)
        return [PublicCandidateRecord.model_validate(_payload(row)) for row in self._fetchall(sql, params)]
    ...
    def get_public_candidate_snapshot(self, snapshot_id: UUID) -> PublicCandidateSnapshot | None:
        row = self._fetchone(
            "select payload from public_candidate_snapshots where snapshot_id = %s",
            (str(snapshot_id),),
        )
        if row is None:
            return None
        return PublicCandidateSnapshot.model_validate(_payload(row))
    ...
    def list_public_candidate_snapshots(
        ...
        *,
        ...
        if limit is not None:
            sql += " limit %s"
            params.append(limit)
        return [PublicCandidateSnapshot.model_validate(_payload(row)) for row in self._fetchall(sql, params)]
    ...
    def list_public_candidate_decision_events(
        ...
        *,
        ...
        if limit is not None:
            sql += " limit %s"
            params.append(limit)
        return [PublicCandidateDecisionEvent.model_validate(_payload(row)) for row in self._fetchall(sql, params)]
