...
class SQLiteResearchRepository(ResearchRepository):
    ...
    def get_collaborator(self, collaborator_id: UUID) -> CollaboratorRecord | None:
        row = self.conn.execute(
            "select payload from collaborators where collaborator_id = ?",
            (str(collaborator_id),),
        ).fetchone()
        return CollaboratorRecord.model_validate(json.loads(row["payload"])) if row else None

    def get_collaborator_by_principal(self, principal: str) -> CollaboratorRecord | None:
        row = self.conn.execute(
            "select payload from collaborators where principal = ? order by updated_at desc limit 1",
            (principal.strip(),),
        ).fetchone()
        return CollaboratorRecord.model_validate(json.loads(row["payload"])) if row else None
    ...
    def list_collaborators(
        ...
        *,
        ...
        if limit is not None:
            ...
            params.append(limit)
        return [
            CollaboratorRecord.model_validate(json.loads(row["payload"]))
            for row in self.conn.execute(sql, params).fetchall()
        ]
    ...
    def get_proof_capsule(self, capsule_id: UUID) -> ProofCapsuleRecord | None:
        row = self.conn.execute(
            "select payload from proof_capsules where capsule_id = ?",
            (str(capsule_id),),
        ).fetchone()
        if row is None:
            return None
        return ProofCapsuleRecord.model_validate(json.loads(row["payload"]))
    ...
    def list_proof_capsules(
        ...
        *,
        ...
        if checkout_manifest_hash:
            clauses.append("checkout_manifest_hash = ?")
        ...
        if requested_action:
            clauses.append("requested_action = ?")
        ...
        if limit is not None:
            ...
            params.append(limit)
        return [
            ProofCapsuleRecord.model_validate(json.loads(row["payload"]))
            for row in self.conn.execute(sql, params).fetchall()
        ]
